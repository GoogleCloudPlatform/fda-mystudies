...
Example:
./push_images.sh gcr.io/valicare-research gcr.io/valicare-research-apps
EOF
  exit 1
fi
...
for image in $(gcloud container images list --repository=gcr.io/valicare-research | tail -n +2); do
