...
Example:
./push_images.sh gcr.io/validcare-research gcr.io/validcare-research-apps
EOF
  exit 1
fi
...
for image in $(gcloud container images list --repository=gcr.io/validcare-research | tail -n +2); do
