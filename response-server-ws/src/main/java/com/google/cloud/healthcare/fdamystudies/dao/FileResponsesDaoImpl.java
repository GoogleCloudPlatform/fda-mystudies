  ...
  public void saveStudyMetadata(String studyCollectionName, String studyId,
    ...
    if (studyCollectionName != null && dataToStore != null) {
      try {
        ...
        Files.write(
            Paths.get(studyDirPath + AppConstants.FILE_SEPARATOR + studyCollectionName
                + AppConstants.JSON_FILE_EXTENSION),
            studyMetadataJsonStr.getBytes(), StandardOpenOption.CREATE,
            StandardOpenOption.TRUNCATE_EXISTING);
        logger.debug("saveStudyMetadata() : \n Study Collection Name: " + studyCollectionName
            + " save successfully. Path to StudyMetadata file: "
            + appConfig.getResponseDataFilePath() + studyCollectionName + studyDirPath
            + AppConstants.FILE_SEPARATOR + studyCollectionName + AppConstants.JSON_FILE_EXTENSION);
  ...
  public void saveActivityResponseData(String studyCollectionName, String studyId,
    ...
    if (studyCollectionName != null && dataToStoreParticipantCollectionMap != null
      ...
      try {
        ...
        logger.error(e.getMessage(), e);
        throw new ProcessResponseException(
            "FileResponsesDaoImpl.saveActivityResponseData() - Exception when saving data to Firestore: "
                + e.getMessage());
      }
    } else {
      throw new ProcessResponseException(
