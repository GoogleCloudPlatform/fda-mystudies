...
public class ActivityResponseProcessorServiceImpl implements ActivityResponseProcessorService {
  ...
  public void saveActivityResponseDataForParticipant(
      ...
      ActivityResponseBean questionnaireActivityResponseBean) throws ProcessResponseException {
    ...
    if (activityMetadataResponse.getActivityId()
      ...
      String rawResponseData = getRawJsonInputData(questionnaireActivityResponseBean);
      ...
      logger.error(
          "saveActivityResponseDataForParticipant() - The activity ID in the response does not match activity ID in the metadata provided.\n"
              + "Activity Id in response: " + activityMetadataResponse.getActivityId() + "\n"
  ...
  private void processActivityResponses(List<QuestionnaireActivityStepsBean> questionnaireResponses,
  ...
  private void plugInMetadataToResponses(
      List<QuestionnaireActivityStepsBean> activityMetadataBeanFromWCP,
      QuestionnaireActivityStepsBean responseBean, boolean fromGrouped) {
    ...
    if (fromGrouped) {
      for (QuestionnaireActivityStepsBean stepBean : activityMetadataBeanFromWCP) {
        ...
        metadataMatchList = stepsBean.stream()
      ...
      metadataMatchList = activityMetadataBeanFromWCP.stream()
          .filter(QuestionnaireActivityStepsBeanPredicate.questionKeyMatch(questionKey))
          .collect(Collectors.<QuestionnaireActivityStepsBean>toList());
    }
  ...
  private void saveActivityResponseData(ActivityResponseBean questionnaireActivityResponseBean,
      String rawResponseData) throws ProcessResponseException {
    try {

      // Add Timestamp to bean
      questionnaireActivityResponseBean
          .setCreatedTimestamp(String.valueOf(System.currentTimeMillis()));

      Map<String, Object> dataToStoreParticipantCollectionMap =
          getMapForParticipantCollection(questionnaireActivityResponseBean);
      ...
      dataToStoreActivityResults.put(AppConstants.CREATED_TS_KEY,
      ...
      String siteId = questionnaireActivityResponseBean.getSiteId();
      String studyCollectionName = AppUtil.makeStudyCollectionName(studyId);
      String participantCollectionName = AppUtil.makeParticipantCollectionName(studyId, siteId);
      String activitiesCollectionName = AppUtil.makeActivitiesCollectionName(studyId, siteId);
      logger.info("saveActivityResponseData() : \n Study Collection Name: " + studyCollectionName);
      responsesDao.saveActivityResponseData(studyId, studyCollectionName, participantCollectionName,
          activitiesCollectionName, dataToStoreParticipantCollectionMap,
  ...
  private Map<String, Object> getMapForParticipantCollection(
    ...
    dataToStoreParticipantCollMap.put(AppConstants.PARTICIPANT_ID_KEY,
        questionnaireActivityResponseBean.getParticipantId());
    dataToStoreParticipantCollMap.put(AppConstants.SITE_ID_KEY,
        questionnaireActivityResponseBean.getSiteId());
    dataToStoreParticipantCollMap.put(AppConstants.SHARING_CONSENT_KEY,
        questionnaireActivityResponseBean.getSharingConsent());
    dataToStoreParticipantCollMap.put(AppConstants.CREATED_TS_KEY,
        questionnaireActivityResponseBean.getCreatedTimestamp());
    ...
    for (PropertyDescriptor pd : propDescriptor) {
      ...
      if (!propertyName.equals(AppConstants.PROPERTY_NAME_CLASS)) {
        ...
        try {
          ...
          logger.debug("getHashMapForBean() : \n Property Name: " + propertyName
              + "\t Propert Value : " + propertyValue);
  ...
  @Override
  public StoredResponseBean getActivityResponseDataForParticipant(String studyId, String siteId,
      String participantId, String activityId) throws ProcessResponseException {
    if (StringUtils.isBlank(studyId)) {
      throw new ProcessResponseException(
          "getActivityResponseDataForParticipant() method: Study Id argument is null or empty.");
    }
    String studyCollectionName = AppUtil.makeStudyCollectionName(studyId);
    return responsesDao.getActivityResponseDataForParticipant(studyCollectionName, studyId, siteId,
