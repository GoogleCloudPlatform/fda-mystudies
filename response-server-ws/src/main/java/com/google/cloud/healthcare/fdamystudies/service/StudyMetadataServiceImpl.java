...
public class StudyMetadataServiceImpl implements StudyMetadataService {
  ...
  public void saveStudyMetadata(StudyMetadataBean studyMetadataBean)
    ...
    try {
      ...
      responsesDao.saveStudyMetadata(studyCollectionName, studyMetadataBean.getStudyId(),
          dataToStore);
      logger.debug("saveStudyMetadata() : \n Study Collection Name: " + studyCollectionName
          + " added successfully");
  ...
  @Override
  public QuestionnaireActivityStructureBean getStudyActivityMetadata(String orgId,
    ...
    try {
      ...
      headers.set(AppConstants.APPLICATION_ID_HEADER, applicationId);
      headers.set(AppConstants.AUTHORIZATION_HEADER, appConfig.getWcpAuthorizationHeader());

      UriComponentsBuilder studyMetadataUriBuilder = UriComponentsBuilder
          .fromHttpUrl(appConfig.getWcpStudyActivityMetadataUrl())
          .queryParam(AppConstants.STUDY_ID_PARAM, studyActivityMetadataRequestBean.getStudyId())
          .queryParam(AppConstants.ACTIVITY_ID_PARAM,
              studyActivityMetadataRequestBean.getActivityId())
          .queryParam(AppConstants.ACTIVITY_VERSION_PARAM,
              studyActivityMetadataRequestBean.getActivityVersion());
      logger.debug(studyMetadataUriBuilder.toUriString());
      responseEntity = restTemplate.exchange(studyMetadataUriBuilder.toUriString(), HttpMethod.GET,
          new HttpEntity<>(headers), QuestionnaireActivityMetaDataBean.class);// QuestionnaireActivityStructureBean.class);
