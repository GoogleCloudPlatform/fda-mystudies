...
<div class="col-sm-10 col-rc white-bg p-none">
	...
	<form:form
		action="/studybuilder/adminStudies/saveOrUpdateInstructionStep.do?_S=${param._S}"
		...
		<div class="right-content-body">
			...
			<div class="col-md-6 pl-none">
				<div class="gray-xs-f mb-xs">
					Step Short Title or Key (15 characters max)<span
			...
			<div class="gray-xs-f mb-xs">
				Title (250 characters max)<span class="requiredStar">*</span>
			</div>
			<div class="form-group">
          ...
          <div class="help-block with-errors red-txt"></div>
			...
			<div class="form-group">
				<textarea class="form-control" id="instructionText"
					name="instructionText" required>${instructionsBo.instructionText}</textarea>
				<div class="help-block with-errors red-txt"></div>
			</div>
			<div class="clearfix"></div>
...
<script type="text/javascript">
				...
				$("#doneId").click(function() {
					...
					validateShortTitle('', function(val) {
						if (val) {
							...
							var isSummernoteValid= maxLenValEditor();
							if (isFromValid("#basicInfoFormId") && isSummernoteValid) {
								var instructionText = $('#instructionText').summernote('code');
								instructionText = replaceSpecialCharacters(instructionText);
				...
				$('#instructionText').summernote(
						{
							...
							toolbar : [ [ 'font', [ 'bold', 'italic' ] ],
									...
									[ 'insert', [ 'link' ] ], [ 'hr' ],
									[ 'clear' ], [ 'cut' ], [ 'undo' ],
									[ 'redo' ], [ 'fontname', [ 'fontname' ] ],
									[ 'fontsize', [ 'fontsize' ] ], ]

						});
				<c:if test="${actionTypeForQuestionPage eq 'view'}">
				$('#instructionText').summernote('disable');
				</c:if>
			});
	function maxLenValEditor() {
		var isValid = true; 
		var value = $('#instructionText').summernote('code');
		if(value != '<p><br></p>'){
			if(value != ''  && $.trim(value.replace(/(<([^>]+)>)/ig, "")).length > 15000 ){
				if(isValid){
					isValid = false;
				}
					$('#instructionText').parent().addClass('has-error-cust').find(".help-block").empty().append('<ul class="list-unstyled"><li>Maximum 15000 characters are allowed.</li></ul>');

			} else {
				$('#instructionText').parent().removeClass("has-danger")
						.removeClass("has-error");
				$('#instructionText').parent().find(".help-block").html("");
			}
		} else {
			isValid = false;
			$('#instructionText')
	...
	function saveInstruction(item) {
		...
		var instruction_text = $('#instructionText').summernote('code');
		if (instruction_text != '<p><br></p>') {
			instruction_text = replaceSpecialCharacters(instruction_text);
		}
		...
		var isSummernoteValid = maxLenValEditor();
		if (isSummernoteValid && (questionnaire_id != null && questionnaire_id != '' && typeof questionnaire_id != 'undefined')
