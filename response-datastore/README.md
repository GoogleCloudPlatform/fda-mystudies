...
1. Configure the `Response datastore` database on the Cloud SQL instance
    -    Create a user account that the `Response datastore` application will use to access this database ([instructions](https://cloud.google.com/sql/docs/mysql/create-manage-users))
...
1. Create a Cloud Firestore database operating in *Native mode* with these [instructions](https://cloud.google.com/firestore/docs/quickstart-servers), then grant the [`roles/datastore.user`](https://cloud.google.com/datastore/docs/access/iam#iam_roles) IAM role to the service account your `Response datastore` will use to read/write data (this could be your VM’s [default service](https://cloud.google.com/compute/docs/access/service-accounts#default_service_account) account)
...
1. Deploy the `Response datastore` container to the VM
    ...
    -    Update the Docker environment file [`variables.env`](variables.env) with values to configure the [`application.properties`](response-server-service/src/main/resources/application.properties) file for your deployment
