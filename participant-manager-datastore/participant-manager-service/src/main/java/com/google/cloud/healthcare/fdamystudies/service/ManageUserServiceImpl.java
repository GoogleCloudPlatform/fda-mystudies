...
public class ManageUserServiceImpl implements ManageUserService {
  ...
  public AdminUserResponse updateUser(
    ...
    // update mobile no in user table of auth-server db
    if (user.isIdpUser()) {
      logger.debug("begin updateAdminPhoneNoInOauth");
      ...
      if (StringUtils.isNotEmpty(adminDetails.getUrAdminAuthId())) {
        String UrAdminAuthId =
            adminDetails.getUrAdminAuthId(); // UrAdminAuthId is userId of User table in auth db
        updateAdminPhoneNoInOauth(UrAdminAuthId, user.getPhoneNum());
      }
    }
    logger.debug("ends updateAdminPhoneNoInOauth");
    //
  ...
  @Override
  public void updateAdminPhoneNoInOauth(String urAdminAuthId, String phoneNum) {
    AuthUserUpdate userRequest = new AuthUserUpdate(urAdminAuthId, phoneNum);

    HttpHeaders headers = new HttpHeaders();
    headers.add("Authorization", "Bearer " + oauthService.getAccessToken());
    // AuditEventMapper.addAuditEventHeaderParams(headers, auditRequest);

    HttpEntity<AuthUserUpdate> requestEntity = new HttpEntity<>(userRequest, headers);
    ResponseEntity<UserResponse> response =
        restTemplate.postForEntity(
            appConfig.getAuthServerUpdateAdminPhoneUrl(), requestEntity, UserResponse.class);

    UserResponse userResponse = response.getBody();
    logger.debug("updateAdminPhoneNoInOauth:", userResponse);
  }
  ...
  public IDPAdminDetailsResponse getIDPAdminDetails(String signedInUserId) {
    ...
    return new IDPAdminDetailsResponse(
        MessageCode.GET_IDP_USERS_SUCCESS,
        idpEmails,
        appConfig.isMfaEnabled(),
        appConfig.isIdpEnabled());
  }
