...
public class TokenIntrospectionFilter extends BaseTokenIntrospectionFilter {
  ...
  public void init() {
    uriTemplateAndMethods.put(
        String.format("%s/studyState", context.getContextPath()),
        new String[] {HttpMethod.GET.name()});
    uriTemplateAndMethods.put(
        String.format("%s/updateStudyState", context.getContextPath()),
        new String[] {HttpMethod.POST.name()});
    uriTemplateAndMethods.put(
        String.format("%s/withdrawfromstudy", context.getContextPath()),
        new String[] {HttpMethod.POST.name()});
    uriTemplateAndMethods.put(
        String.format("%s/validateEnrollmentToken", context.getContextPath()),
        new String[] {HttpMethod.POST.name()});
    uriTemplateAndMethods.put(
        String.format("%s/enroll", context.getContextPath()),
        new String[] {HttpMethod.POST.name()});
    uriTemplateAndMethods.put(
        String.format("%s/participantInfo", context.getContextPath()),
        new String[] {HttpMethod.GET.name()});
  }
