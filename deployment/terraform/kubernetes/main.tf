...
data "google_secret_manager_secret_version" "secrets" {
  ...
  project  = "btc-terradeploy-secrets"
  secret   = each.key
...
resource "kubernetes_secret" "shared_secrets" {
  ...
  data = {
    consent_bucket_name               = "btc-terradeploy-mystudies-consent-documents"
    study_resources_bucket_name       = "btc-terradeploy-mystudies-study-resources"
...
resource "google_service_account_key" "apps_service_account_keys" {
  ...
  service_account_id = "${each.key}@btc-terradeploy-apps.iam.gserviceaccount.com"
}
